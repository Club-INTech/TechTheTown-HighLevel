...
public class ObstacleManager implements Service
{
        ...
        if (isObstaclePositionValid(position))
        {
            ...
            for (int i=0; i<mUntestedMobileObstacles.size(); i++)
            {
                ...
                if (obstacleMobileUntested.getPosition().distance(position) < obstacleMobileUntested.getRadius()/2)
                {
                    ...
                    obstacleMobileUntested.setRadius(radius + mRobotRadius);
            ...
            for (int i=0; i<mMobileObstacles.size(); i++)
            {
                ...
                if (obstacleMobile.getPosition().distance(position) < obstacleMobile.getRadius()/2)
                {
                    ...
                    obstacleMobile.setRadius(radius + mRobotRadius);
            ...
            if (!isThereAnObstacleIntersecting) {
                mUntestedMobileObstacles.add(new ObstacleProximity(new Circle(position, radius + mRobotRadius), timeToTestObstacle));
            }
    ...
    public synchronized Obstacle getClosestObstacle (Vec2 position, Vec2 direction){
        ...
        try {
            ...
            for (int i = 0; i < mCircularObstacle.size(); i++) {
                ...
                distanceToObstacleTested = obstacleRelativeCoords.length() - mCircularObstacle.get(i).getRadius()/2;
    ...
    public boolean isEnnemyForwardOrBackWard(int distance, Vec2 pos, Vec2 aim, double orientation){
        ...
        if (Math.abs(ennemyPos.getY()) < obstacleRadius &&
