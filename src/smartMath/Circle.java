...
public class Circle {
	...
	public ArrayList<Vec2> pointsaroundcircle(int n){
		...
		for(int i=0;i<n;i++){
			int x=Integer.valueOf((int)(this.getRadius()*Math.cos(2*i*Math.PI/n))+this.getCenter().getX());
			int y=Integer.valueOf((int)(this.getRadius()*Math.sin(2*i*Math.PI/n))+this.getCenter().getY());
